...
const metadata_version_line = std.fmt.comptimePrint(
    "Bun {s}v{s} {s} {s}{s}\n",
    .{
        if (bun.Environment.isDebug) "Debug " else if (bun.Environment.is_build) "Build " else if (bun.Environment.is_canary) "Canary " else "",
...
pub const js_bindings = struct {
    ...
    pub fn jsGetFeatureData(global: *JSC.JSGlobalObject, _: *JSC.CallFrame) JSC.JSValue {
        ...
        obj.put(global, JSC.ZigString.static("is_build"), JSC.JSValue.jsBoolean(bun.Environment.is_build));
        obj.put(global, JSC.ZigString.static("build_number"), JSValue.jsNumberFromInt64(bun.Environment.build_number));
