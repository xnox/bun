...
const [H2FrameParser, getPackedSettings, getUnpackedSettings] = $zig(
	"h2_frame_parser.zig",
		...
		if (sensitives) {
			if (!$isJSArray(sensitives)) {
				const error = new TypeError(
					"ERR_INVALID_ARG_VALUE: The argument headers[http2.neverIndex] is invalid",
				);
	...
	respond(headers: any, options?: any) {
		...
		if (sensitives) {
			if (!$isArray(sensitives)) {
				const error = new TypeError(
					"ERR_INVALID_ARG_VALUE: The arguments headers[http2.neverIndex] is invalid",
				);
				error.code = "ERR_INVALID_ARG_VALUE";
				throw error;
			}
    ...
    if(headers[":status"] === undefined) {
      headers[":status"] = 200;
		...
		if (typeof options === "undefined") {
			session[bunHTTP2Native]?.request(this.id, headers, sensitiveNames);
		} else {
			session[bunHTTP2Native]?.request(this.id, headers, sensitiveNames, options);
		}
		return;
	}
}

function connectWithProtocol(
	protocol: string,
	options: Http2ConnectOptions | string | URL,
	listener?: Function,
) {
	if (protocol === "http:") {
...
class ServerHttp2Session extends Http2Session {
	...
	static #Handlers = {
		binaryType: "buffer",
		streamStart(self: ServerHttp2Session, streamId: number) {
			if (!self) return;
			self.#connections++;
		},
		streamError(self: ServerHttp2Session, streamId: number, error: number) {
			if (!self) return;
		...
		streamEnd(self: ServerHttp2Session, streamId: number) {
			if (!self) return;
			var stream = self.#streams.get(streamId);
			if (stream) {
				self.#connections--;
		...
		streamData(self: ServerHttp2Session, streamId: number, data: Buffer) {
			if (!self) return;
			var stream = self.#streams.get(streamId);
			if (stream) {
		...
		streamHeaders(
			...
			if (!self) return;
			var stream = new ServerHttp2Stream(streamId, self, headers);
		...
		aborted(self: ServerHttp2Session, streamId: number, error: any) {
			if (!self) return;
			var stream = self.#streams.get(streamId);
			if (stream) {
				if (!stream[bunHTTP2Closed]) {
		...
		wantTrailers(self: ServerHttp2Session, streamId: number) {
			if (!self) return;
			var stream = self.#streams.get(streamId);
			if (stream) {
				stream[bunHTTP2WantTrailers] = true;
	...
	constructor(
		...
		if (socket instanceof TLSSocket) {
			...
			const origin =
				socket[bunTLSConnectOptions]?.serverName || socket.remoteAddress;
	...
	ping(payload, callback) {
		...
		if (!(payload instanceof Buffer) && !isTypedArray(payload)) {
			const error = new TypeError(
				"ERR_INVALID_ARG_TYPE: payload must be a Buffer or TypedArray",
			);
...
class ClientHttp2Session extends Http2Session {
	...
	request(headers: any, options?: any) {
		...
		if (typeof options === "undefined") {
			stream_id = this.#parser.request(undefined, headers, sensitiveNames);
		} else {
			stream_id = this.#parser.request(undefined, headers, sensitiveNames, options);
		}
